...
def search_listings(slots: dict) -> tuple[pd.DataFrame, str]:
    ...
    # 지역 필터: '지역 구' 또는 '지역 동' 어느 쪽이든 매칭
    지역 = slots.get("지역")
    if 지역:
        q = str(지역).replace("서울", "").strip()
        mask_구 = df["지역 구"].astype(str).apply(lambda v: q in v.replace("서울","").strip() or v.replace("서울","").strip() in q)
        mask_동 = df["지역 동"].astype(str).apply(lambda v: q in v.replace("서울","").strip() or v.replace("서울","").strip() in q) \
                  if "지역 동" in df.columns else pd.Series(False, index=df.index)
        df = df[mask_구 | mask_동]
...
def _rule_based(utterance: str) -> dict:
    ...
    for r in CATEGORICAL_SLOTS["지역구"]:
        ...
        for alias, region in _REGION_ALIASES.items():
            ...
            # 구 이름도 별칭도 없으면 → 발화에서 동/구/역 단위로 보이는 표현을 그대로 지역에 넣음
            # listing_search.py가 '지역 구' OR '지역 동' 둘 다 비교하므로 그냥 넘겨도 됨
            m = re.search(r"([가-힣]+(?:동|구|역|로|거리))", u)
            if m:
                s["지역"] = m.group(1)
...
JSON: {"지역": "성동구", "거래유형": "월세", "가격_최대": 50, "기타": "반려동물 가능"}

발화: "마포구 원룸 전세 보증금 1억 이하로 역세권이면 좋겠어요"
JSON: {"지역": "마포구", "거래유형": "전세", "주거유형": "원룸", "보증금_최대": 10000, "근접시설": "역세권"}

발화: "노원구 월세 50 이하 주차되는 조용한 투룸 구해요"
JSON: {"지역": "노원구", "거래유형": "월세", "주거유형": "투룸", "가격_최대": 50, "기타": "주차 가능, 조용한 동네"}

발화: "당장 이번 주에 바로 들어갈 수 있는 강남구 오피스텔 있나요"
JSON: {"지역": "강남구", "주거유형": "오피스텔", "기타": "즉시 입주"}

발화: "송파구 고층 전세 15평 이상"
JSON: {"지역": "송파구", "거래유형": "전세", "층조건": "고층", "면적_최소": 15}
...
"""
        ...
        prompt = (
            ...
            "언급되지 않은 슬롯은 생략. "
            "가격은 만원 단위 정수. "
            "'보증금 N 이하'는 보증금_최대, '월세 N 이하'는 가격_최대로 구분하라. "
            "반려동물/주차/조용함 등 자유조건은 '기타'에 짧게 요약하라. "
            "애매하면 억지로 채우지 말고 생략하라.\n"
